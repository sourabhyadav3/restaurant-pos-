...
const CustomerOrderNow = () => {
 ...
 <button onClick={() => removeFromCart(item.id)} className="p-1 text-slate-300 ">
 ...
 <button onClick={() => updateCartQuantity(item.id, -1)} className="text-slate-400"><Minus className="w-3 h-3" /></button>
 <span className="text-[10px] font-black w-2 text-center">{item.quantity}</span>
 <button onClick={() => updateCartQuantity(item.id, 1)} className="text-slate-400"><Plus className="w-3 h-3" /></button>
 </div>
 </div>
 </div>
 </div>
 ...
 className="w-full btn-primary py-4 rounded-2xl flex items-center justify-center gap-3 text-xs font-black uppercase tracking-widest shadow-xl shadow-primary/20 group"
 >
 ...
 className="w-full pl-11 pr-5 py-3 bg-white border border-slate-100 rounded-2xl outline-none shadow-sm text-sm font-bold focus:ring-4 focus:ring-primary/5"
 />
 </div>
 </div>
 ...
 className={cn(
 "px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest border-2 whitespace-nowrap",
 activeCategory === cat 
 ? "bg-primary text-white border-primary shadow-lg shadow-primary/10" 
 : "bg-white text-text-secondary border-transparent"
 )}
 >
 {cat}
 </button>
 ...
 <div className="flex-1 overflow-y-auto scrollbar-hide pr-1">
  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-4 lg:gap-6 pb-24 lg:pb-6">
 {filteredItems.map(item => (
 <div 
 key={item.id} 
 onClick={() => openItemModal(item)}
 className="card cursor-pointer border-none shadow-xl shadow-slate-100/50 p-4 bg-white flex flex-col h-full relative"
 >
 ...
 <div className="h-40 bg-slate-50 rounded-[2rem] flex items-center justify-center text-6xl mb-4 shadow-inner relative overflow-hidden ">
 {item.image}
 <button 
 onClick={(e) => handleToggleFavorite(item.id, e)}
 className={cn(
 "absolute top-4 right-4 p-2.5 rounded-xl shadow-lg",
 favorites.includes(item.id) ? "bg-rose-500 text-white shadow-rose-200" : "bg-white/90 text-slate-300"
 )}
 >
 ...
 <h4 className="font-black text-text-primary text-sm uppercase tracking-tight leading-tight ">{item.name}</h4>
 ...
 className="w-full h-16 bg-primary text-white rounded-2xl flex items-center justify-between px-6 shadow-2xl shadow-primary/40 "
 >
 ...
 <div className="relative w-full max-w-lg bg-white rounded-[2rem] lg:rounded-[3rem] shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in fade-in zoom-in ">
 <div className="relative h-48 lg:h-56 bg-slate-50 flex items-center justify-center text-6xl lg:text-8xl shrink-0">
 {selectedItem.image}
 <button onClick={() => setShowItemModal(false)} className="absolute top-4 lg:top-6 right-4 lg:right-6 p-2 lg:p-3 bg-white/80 backdrop-blur-md rounded-2xl text-slate-400 shadow-xl ">
 ...
 className={cn(
 "flex-1 min-w-[100px] px-4 py-3 rounded-2xl border-2 text-center",
 ...
 <button onClick={() => setQuantity(quantity + 1)} className="w-10 h-10 bg-white rounded-xl flex items-center justify-center text-slate-400 shadow-sm"><Plus className="w-4 h-4" /></button>
 </div>
 </div>
 <div className="space-y-4">
 ...
 className="w-full btn-primary py-4 rounded-2xl lg:rounded-3xl flex items-center justify-center gap-3 text-sm font-black uppercase tracking-widest shadow-2xl shadow-primary/30 "
 >
