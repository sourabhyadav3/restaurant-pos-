...
const CustomerFavorites = () => {
 ...
 <div key={item.id} className="card p-5 bg-white border-none shadow-xl shadow-slate-100/50 flex flex-col h-full relative">
 <button 
 onClick={() => toggleFavorite(item.id)}
 className="absolute top-4 right-4 z-10 p-2 bg-rose-500 text-white rounded-xl shadow-lg shadow-rose-200 "
 >
 <Heart className="w-4 h-4 fill-current" />
 </button>
 <div className="h-40 bg-slate-50 rounded-2xl flex items-center justify-center text-5xl mb-4 shadow-inner ">
 {item.image}
 </div>
 ...
 className="w-10 h-10 bg-primary/10 text-primary rounded-xl flex items-center justify-center shadow-sm"
 >
