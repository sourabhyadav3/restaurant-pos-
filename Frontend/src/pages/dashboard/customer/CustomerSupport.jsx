...
const CustomerSupport = () => {
  ...
  return (
    <div className="space-y-6 lg:space-y-8 pb-20">
      ...
      {toast && (
        <div className={cn(
          "fixed top-4 left-1/2 -translate-x-1/2 z-[300] px-6 py-3 rounded-2xl shadow-2xl flex items-center gap-3 font-black text-[10px] uppercase tracking-widest border animate-in slide-in-from-top-4 duration-300",
        ...
        <div className="fixed inset-0 z-[500] flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-[500px] bg-white rounded-t-[2.5rem] sm:rounded-[2.5rem] shadow-2xl overflow-hidden animate-in slide-in-from-bottom-full sm:slide-in-from-bottom-10 sm:zoom-in-95 duration-300 self-end sm:self-center">
               <div className="px-8 py-6 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
                  <div>
                    ...
                    <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1">Average response: 15 mins</p>
                  </div>
               ...
               <form onSubmit={handleTicketSubmit} className="p-8 space-y-5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      ...
                      <select 
                        ...
                        className="w-full px-5 py-3 bg-slate-50 border-2 border-transparent focus:border-primary/20 focus:bg-white rounded-2xl outline-none font-bold text-xs appearance-none"
                      >
                  ...
                  <div className="space-y-1.5">
                    ...
                    <textarea 
                      ...
                      className="w-full px-5 py-4 bg-slate-50 border-2 border-transparent focus:border-primary/20 focus:bg-white rounded-2xl outline-none font-bold text-xs min-h-[120px] resize-none"
                    />
                  </div>
                  <div className="pt-2 flex gap-4">
                    ...
                    <button type="submit" className="flex-1 py-4 bg-primary text-white rounded-2xl font-black uppercase tracking-widest text-[10px] shadow-xl shadow-primary/30 active:scale-[0.98] transition-all">Submit Ticket</button>
                  </div>
               </form>
            </div>
          )}
            ...
            <div className="relative w-full max-w-[450px] bg-white rounded-t-[2.5rem] sm:rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col h-[85vh] sm:h-[600px] animate-in slide-in-from-bottom-full sm:slide-in-from-bottom-10 duration-300 self-end sm:self-center">
               <div className="px-6 py-5 border-b border-slate-50 flex justify-between items-center bg-primary text-white">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-base font-black uppercase tracking-tight">Live Concierge</h3>
                      <p className="text-[8px] font-bold uppercase tracking-[0.2em] opacity-70 flex items-center gap-1.5">
                        <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" /> Online
                      </p>
                    </div>
                  </div>
                  <button onClick={() => setActiveModal(null)} className="p-2 hover:bg-white/10 rounded-xl transition-all"><X className="w-5 h-5" /></button>
               </div>
                  ...
                  {chatHistory.map((msg, i) => (
                    <div key={i} className={cn(
                      ...
                      msg.role === 'user' ? "ml-auto items-end" : "items-start"
                       ...
                       <div className={cn(
                         "px-4 py-3 rounded-2xl text-[11px] font-medium shadow-sm",
                         msg.role === 'user' ? "bg-primary text-white rounded-tr-none" : "bg-white text-slate-700 rounded-tl-none"
                       )}>
                         {msg.text}
                       </div>
               ...
               <form onSubmit={handleSendMessage} className="p-4 bg-white border-t border-slate-50 flex gap-2 items-center">
                  <input 
                    type="text"
                    value={chatMessage}
                    onChange={(e) => setChatMessage(e.target.value)}
                    placeholder="Type your message..."
                    className="flex-1 px-5 py-3.5 bg-slate-50 border-2 border-transparent focus:border-primary/20 focus:bg-white rounded-2xl outline-none font-bold text-xs"
                  />
                  <button type="submit" className="w-12 h-12 bg-primary text-white rounded-xl flex items-center justify-center shadow-lg shadow-primary/20 active:scale-90 transition-all">
          ...
          {activeModal === 'call' && (
            <div className="relative w-full max-w-[400px] bg-white rounded-t-[2.5rem] sm:rounded-[2.5rem] shadow-2xl p-8 text-center space-y-6 animate-in slide-in-from-bottom-full sm:slide-in-from-bottom-10 sm:zoom-in-95 duration-300 self-end sm:self-center">
               ...
               <div>
                 <h3 className="text-xl font-black uppercase tracking-tight">Call Support</h3>
                 <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-2 leading-relaxed">You are about to dial our premium support line.</p>
               </div>
          ...
          {activeModal === 'email' && (
            <div className="relative w-full max-w-[400px] bg-white rounded-t-[2.5rem] sm:rounded-[2.5rem] shadow-2xl p-8 text-center space-y-6 animate-in slide-in-from-bottom-full sm:slide-in-from-bottom-10 sm:zoom-in-95 duration-300 self-end sm:self-center">
               ...
               <div>
                 <h3 className="text-xl font-black uppercase tracking-tight">Email Support</h3>
                 <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-2 leading-relaxed">Our email agents respond within 2-4 hours.</p>
               </div>
               <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100">
            ...
            <div className="relative w-full max-w-[500px] bg-white rounded-t-[2.5rem] sm:rounded-[2.5rem] shadow-2xl overflow-hidden animate-in slide-in-from-bottom-full sm:slide-in-from-bottom-10 sm:zoom-in-95 duration-300 self-end sm:self-center">
               <div className="px-8 py-6 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-base font-black uppercase tracking-tight">{selectedTicket.id}</h3>
               ...
               <div className="p-8 space-y-6">
                  <div className="grid grid-cols-2 gap-4">
                     <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
                        ...
                        <p className="text-xs font-black text-text-primary uppercase tracking-tight">{selectedTicket.category || 'Support'}</p>
                     </div>
                     <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
                  ...
                  <div>
                    <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-2 px-1">Subject</p>
                    <p className="text-sm font-black text-text-primary uppercase tracking-tight bg-slate-50 p-4 rounded-2xl border border-slate-100">
                      {selectedTicket.subject || selectedTicket.type}
                    </p>
                  </div>
                  <div>
                    <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-2 px-1">Message Content</p>
                    <div className="bg-white p-4 rounded-2xl border border-slate-100 min-h-[100px] text-xs font-medium text-slate-600 leading-relaxed">
                  ...
                  <div className="flex items-center gap-3 p-4 bg-emerald-50 text-emerald-600 rounded-2xl border border-emerald-100">
                     ...
                     <p className="text-[9px] font-black uppercase tracking-widest">Awaiting staff response • Estimated time: 10m</p>
                  </div>
                  <button onClick={() => setActiveModal(null)} className="w-full py-4 bg-slate-900 text-white rounded-2xl font-black uppercase tracking-widest text-[10px] shadow-xl shadow-slate-900/20">
                     Close Ticket View
                  </button>
               </div>
            </div>
